...
- `agent-ui`：受保护的 Agent 工作台 proposal 请求与前端展示状态。页面不发送审批请求。
...
3. Agent 工作台：访问 `/search-agent.html` 并完成服务器鉴权，再开启 `agent-ui` 检查 proposal 请求。控制台只记录 proposal ID、状态和错误码，不记录凭据、Authorization 或完整响应；遇到 `401` 时刷新页面重新登录。策略审批仅在服务器后台执行。
